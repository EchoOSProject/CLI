const { existsSync, PathLike } = require("fs");
require("colors");
...
module.exports = class {
  ...
  boot(id) {
    return new Promise((resolve, reject) => {
      if (
        require(__dirname + "/../systems/identifiers.json") !==
        (undefined || null)
      ) {
        require(__dirname + "/../systems/identifiers.json").forEach((sys) => {
          if (sys.id == id) {
            if (sys.entry !== ("" || null || undefined)) {
              if (
                require(__dirname + `/../systems/${sys.id}/${sys.entry}`)
                  .__init !== (undefined || null) ||
                typeof require(__dirname + `/../systems/${sys.id}/${sys.entry}`)
                  .__init == "function"
              ) {
                resolve("Done!");
                try {
                  ...
                  require(__dirname +
                    `/../systems/${sys.id}/${sys.entry}`).__init();
                }
              } else {
                reject(
                  new Error(
